  ...
  if (!reportData) {
    return (
      <View className="flex-1 bg-gray-100 items-center justify-center">
        <Text className="text-gray-600">리포트 데이터를 불러오는 중...</Text>
      </View>
    );
  }
  ...
  return (
    <View className="flex-1 bg-[#0A0E27]" style={{ paddingTop: insets.top }}>
      ...
      <View className="flex-row items-center justify-between px-4 py-3">
        ...
        <Text className="text-base font-semibold text-white">분석 리포트</Text>
        <View className="w-11" />
      </View>
        ...
        {currentPage === 1 && (
          <Animated.View entering={FadeIn.duration(300)} className="gap-4">
            ...
            <View className="bg-[#1A1F3D] rounded-2xl p-6 items-center border border-white/10">
              <Text className="text-xs font-bold text-green-500 mb-2 tracking-widest">
                {cropName}{varietyName ? `(${varietyName})` : ''} 분석 리포트
              </Text>
              <Text className="text-sm font-semibold text-white/80 mb-4">수정벌 관리 최적화 지수</Text>

              <View className="w-36 h-36 rounded-full border-8 border-white/10 items-center justify-center mb-4">
                <Text className="text-4xl font-bold text-white">{overallScore}</Text>
                <Text className="text-xs text-white/75">점</Text>
              </View>

              <Text className="text-sm text-white/80">
                상위 <Text className="text-green-500 font-bold">{Math.max(1, 100 - Math.round(overallScore * 0.8))}%</Text>입니다
              </Text>
            </View>
            ...
            {beeRecommendations.length > 0 && (
              <View className="bg-white rounded-2xl p-5">
                <View className="flex-row items-center gap-2 mb-4">
                  ...
                  <Text className="text-sm font-bold text-gray-900">1. 맞춤 수정벌 추천</Text>
                </View>
                ...
                <View className="bg-green-50 rounded-xl p-4 border-2 border-green-400 mb-3">
                  ...
                  <View className="flex-row items-center justify-between mb-3 pt-1">
                    <Text className="text-base font-bold text-green-600">{beeRecommendations[0].name}</Text>
                    <Text className="text-base font-bold text-gray-900">
                      {beeRecommendations[0].price.toLocaleString()}원
                      <Text className="text-xs font-normal text-gray-600">/박스</Text>
                    </Text>
                  </View>
                  <View className="flex-row gap-2 mb-3">
                    <View className="flex-1 bg-white/80 rounded-lg p-2 items-center">
                      <Text className="text-xs text-gray-600 mb-0.5">교체 주기</Text>
                      <Text className="font-bold text-gray-900">{beeRecommendations[0].lifespan}</Text>
                    </View>
                    <View className="flex-1 bg-white/80 rounded-lg p-2 items-center">
                      <Text className="text-xs text-gray-600 mb-0.5">{cropName} 적합도</Text>
                      <Text className="font-bold text-green-600">{beeRecommendations[0].cropMatch}%</Text>
                    </View>
                    <View className="flex-1 bg-white/80 rounded-lg p-2 items-center">
                      <Text className="text-xs text-gray-600 mb-0.5">적정 온도</Text>
                      <Text className="font-bold text-gray-900">{beeRecommendations[0].tempRange}</Text>
                    </View>
                  </View>
                  <View className="flex-row flex-wrap gap-1">
                    {beeRecommendations[0].features.map((f, i) => (
                      <View key={i} className="bg-green-200 px-2 py-0.5 rounded-full">
                        <Text className="text-xs text-green-700 font-medium">{f}</Text>
                      </View>
                    ))}
                  </View>
                </View>
                  ...
                  {beeRecommendations.slice(1).map((bee) => (
                    <View key={bee.rank} className="flex-1 bg-gray-50 rounded-xl p-3 border border-gray-200">
                      <View className="absolute -top-2 left-3 bg-gray-500 px-1.5 py-0.5 rounded-full">
                        <Text className="text-xs font-bold text-white">{bee.rank}위</Text>
                      </View>
                      <Text className="text-xs font-bold text-gray-700 pt-1 mb-2">{bee.name}</Text>
                      <View className="gap-1">
                        <View className="flex-row justify-between">
                          <Text className="text-xs text-gray-600">가격</Text>
                          <Text className="text-xs font-semibold text-gray-800">{bee.price.toLocaleString()}원</Text>
                        </View>
                        <View className="flex-row justify-between">
                          <Text className="text-xs text-gray-600">교체</Text>
                          <Text className="text-xs font-semibold text-gray-800">{bee.lifespan}</Text>
                        </View>
                        <View className="flex-row justify-between">
                          <Text className="text-xs text-gray-600">적합도</Text>
                          <Text className="text-xs font-semibold text-gray-800">{bee.cropMatch}%</Text>
                        </View>
                      </View>
                    </View>
                  ))}
            ...
            {hasEnvData && (
              <View className="bg-white rounded-2xl p-5">
                <View className="flex-row items-center gap-2 mb-4">
                  ...
                  <Text className="text-sm font-bold text-gray-900">2. 온·습도 상태 진단</Text>
                </View>

                <View className="flex-row gap-3 mb-3">
                  <View className="flex-1 bg-gray-50 rounded-xl p-4 items-center">
                    <Feather name="thermometer" size={20} color="#FFD55F" />
                    <Text className="text-xs text-gray-600 mt-2 mb-1">평균 온도</Text>
                    <Text className={`text-xl font-bold ${tempOptimal ? 'text-green-600' : 'text-yellow-500'}`}>
                      {input?.averageTemperature}°C
                    </Text>
                    <Text className="text-xs text-gray-600 mt-1">벌 최적: {beeRecommendations[0]?.tempRange || '18~25°C'}</Text>
                  </View>
                  <View className="flex-1 bg-gray-50 rounded-xl p-4 items-center">
                    <Feather name="droplet" size={20} color="#3B82F6" />
                    <Text className="text-xs text-gray-600 mt-2 mb-1">평균 습도</Text>
                    <Text className={`text-xl font-bold ${humidOptimal ? 'text-green-600' : 'text-red-500'}`}>
                      {input?.averageHumidity}%
                    </Text>
                    <Text className="text-xs text-gray-600 mt-1">벌 최적: {managementGuide?.humidityControl?.recommendedRange || '50~70%'}</Text>
                  </View>
                </View>
                ...
                {(tempHigh || humidHigh) && (
                  <View className="bg-red-50 rounded-xl p-3 border border-red-100">
                    <View className="flex-row items-center gap-1.5 mb-1">
                      ...
                      <Text className="text-xs font-bold text-red-600">문제 진단</Text>
                    </View>
                    {tempHigh && <Text className="text-xs text-red-600/80">• 고온으로 벌 비행 시간 단축</Text>}
                    {humidHigh && <Text className="text-xs text-red-600/80">• 고습으로 꽃가루 점착, 수정 효율 하락</Text>}
                  </View>
                )}
              </View>
            )}
          ...
          <Animated.View entering={FadeIn.duration(300)} className="gap-4">
            ...
            {priorityImprovements.length > 0 && (
              <View className="bg-gradient-to-br rounded-2xl p-5 border border-red-500/20" style={{ backgroundColor: 'rgba(239, 68, 68, 0.1)' }}>
                <View className="flex-row items-center gap-2 mb-4">
                  ...
                  <Text className="text-sm font-bold text-white">우선 개선 사항 (시급 순)</Text>
                </View>
                  ...
                  {priorityImprovements.map((item, idx) => (
                    <View key={idx} className="bg-white/10 rounded-xl p-3 flex-row items-start gap-3 border border-white/10">
                      ...
                      <View className="flex-1">
                        <Text className="text-sm font-bold text-white">{item.issue}</Text>
                        <Text className="text-xs text-white/70">{item.reason}</Text>
                      </View>
                    </View>
                  ))}
                </View>
            ...
            <View className="bg-white rounded-2xl p-5">
              <View className="flex-row items-center gap-2 mb-4">
                ...
                <Text className="text-sm font-bold text-gray-900">3. 온도 관리</Text>
              </View>
              ...
              <View className="bg-yellow-50 rounded-xl p-4 border border-yellow-200">
                <View className="flex-row gap-3 mb-3">
                  <View className="flex-1 bg-white rounded-lg p-3 border border-gray-100">
                    <Text className="font-semibold text-gray-900 mb-1 text-xs">벌 최적 온도</Text>
                    <Text className="text-lg font-bold text-yellow-500">{beeRecommendations[0]?.tempRange || '18~25°C'}</Text>
                  </View>
                  <View className="flex-1 bg-white rounded-lg p-3 border border-gray-100">
                    <Text className="font-semibold text-gray-900 mb-1 text-xs">{cropName} 최적 온도</Text>
                    <Text className="text-lg font-bold text-green-600">{managementGuide?.temperatureControl?.recommendedRange || '15~25°C'}</Text>
                  </View>
                </View>
                <View className="gap-1.5">
                  {(managementGuide?.temperatureControl?.actions || ['오전 9~11시 개화 시간대 환기 강화', '고온 시(25°C 초과) 차광막 활용']).map((action, idx) => (
                    <View key={idx} className="flex-row items-start gap-2">
                      ...
                      <Text className="flex-1 text-xs text-gray-600">{action}</Text>
                    </View>
                  ))}
                </View>
              </View>
            ...
            <View className="bg-white rounded-2xl p-5">
              <View className="flex-row items-center gap-2 mb-4">
                ...
                <Text className="text-sm font-bold text-gray-900">4. 습도 관리</Text>
              </View>

              <View className="bg-blue-50 rounded-xl p-4 border border-blue-200">
                <View className="flex-row gap-3 mb-3">
                  <View className="flex-1 bg-white rounded-lg p-3 border border-gray-100">
                    <Text className="font-semibold text-gray-900 mb-1 text-xs">벌 최적 습도</Text>
                    <Text className="text-lg font-bold text-blue-500">{managementGuide?.humidityControl?.recommendedRange || '50~70%'}</Text>
                  </View>
                  <View className="flex-1 bg-white rounded-lg p-3 border border-gray-100">
                    <Text className="font-semibold text-gray-900 mb-1 text-xs">{cropName} 최적 습도</Text>
            ...
            <View className="bg-white rounded-2xl p-5">
              <View className="flex-row items-center gap-2 mb-4">
                ...
                <Text className="text-sm font-bold text-gray-900">5. 벌 관리</Text>
              </View>
              ...
              <View className="gap-3">
                ...
                <View className="bg-green-50 rounded-xl p-4 border border-green-200">
                  <View className="flex-row items-center gap-2 mb-2">
                    ...
                    <Text className="text-sm font-bold text-gray-900">벌통 위치</Text>
                  </View>
                ...
                <View className="bg-green-50 rounded-xl p-4 border border-green-200">
                  <View className="flex-row items-center gap-2 mb-2">
                    ...
                    <Text className="text-sm font-bold text-gray-900">교체 주기</Text>
                  </View>
                  <Text className="text-xs text-gray-700">
                ...
                <View className="bg-green-50 rounded-xl p-4 border border-green-200">
                  <View className="flex-row items-center gap-2 mb-2">
                    ...
                    <Text className="text-sm font-bold text-gray-900">밀도 보정</Text>
                  </View>
                  <Text className="text-xs text-gray-700">
                    현재 동당 {boxesPerHouse}박스 → <Text className="font-bold text-green-600">동당 {boxesPerHouse + 1}박스</Text> 권장
                  </Text>
          ...
          <Animated.View entering={FadeIn.duration(300)} className="gap-4">
            ...
            <View className="bg-[#1A1F3D] rounded-2xl p-5 border border-white/10">
              <View className="flex-row items-center gap-2 mb-4">
                ...
                <Text className="text-sm font-bold text-white">6. 기대 수익률 분석</Text>
              </View>
              ...
              <View className="bg-white/5 rounded-xl p-4 mb-4 border border-white/10">
                <Text className="text-xs text-white/75 mb-2">{cropName} 현재 시세</Text>
                <Text className="text-2xl font-bold text-yellow-400">
                  {marketPrice.toLocaleString()}원<Text className="text-sm font-normal text-white/75">/kg</Text>
                </Text>
              </View>
              ...
              <View className="flex-row gap-2 mb-4">
                <View className="flex-1 bg-white/5 rounded-lg p-3 items-center">
                  <Text className="text-xs text-white/70 mb-1">연간 생산량</Text>
                  <Text className="text-lg font-bold text-white">{(input?.annualKg || 5000).toLocaleString()}kg</Text>
                </View>
                <View className="flex-1 bg-white/5 rounded-lg p-3 items-center">
                  <Text className="text-xs text-white/70 mb-1">현재 착과율</Text>
                  <Text className="text-lg font-bold text-white/80">{revenueAnalysis?.currentRate || 85}%</Text>
                </View>
                <View className="flex-1 bg-green-500/20 rounded-lg p-3 items-center border border-green-500/30">
                  <Text className="text-xs text-green-400 mb-1">개선 후</Text>
                  <Text className="text-lg font-bold text-green-400">{revenueAnalysis?.improvedRate || 95}%</Text>
                </View>
              </View>
              ...
              <View className="flex-row gap-3 mb-4">
                <View className="flex-1 bg-white/5 rounded-xl p-3">
                  <Text className="text-xs text-white/70 mb-1">추가 매출</Text>
                  <Text className="text-xl font-bold text-green-400">+{formatCurrency(revenueAnalysis?.additionalRevenue || 0)}원</Text>
                </View>
                <View className="flex-1 bg-white/5 rounded-xl p-3">
                  <Text className="text-xs text-white/70 mb-1">추가 비용</Text>
              ...
              <View className="bg-green-500/10 rounded-xl p-4 flex-row items-center justify-between border border-green-500/30">
                <View>
                  <Text className="text-xs text-white/70 mb-1">기대 순이익</Text>
                ...
                <View className="items-end">
                  <Text className="text-xs text-white/70 mb-1">기대 ROI</Text>
            ...
            <View className="bg-[#1A1F3D] rounded-2xl p-5 border border-green-500/30">
              <View className="flex-row items-start gap-4">
                ...
                <View className="flex-1">
                  <Text className="text-base font-bold text-white mb-2">최종 결론</Text>
                  <Text className="text-sm text-white/90 mb-1">
                    현재 <Text className="font-bold text-white">{brandName} 수정벌</Text>은 {cropName}{varietyName ? `(${varietyName})` : ''}에 {finalConclusion?.suitability || '적합'}합니다.
                  </Text>
                  <Text className="text-sm text-white/80 mb-2">온·습도 조정 및 박스 밀도 보정 시</Text>
                  <Text className="text-lg font-bold text-white">
                    착과율 <Text className="text-yellow-400">
                  ...
                  <Text className="text-lg font-bold text-white">
                    연간 기대 수익 <Text className="text-yellow-400">
      ...
      >
        ...
        >
          ...
          <Text className="text-white/70 font-semibold ml-1">
        ...
        {currentPage < totalPages ? (
          ...
          >
            <Text className="text-black font-bold">진단 완료</Text>
          </Pressable>
        )}
      </View>
    </View>
