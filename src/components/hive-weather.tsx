...
import { View, Pressable, ActivityIndicator, Linking, Platform, Text } from "react-native";
import { Feather } from "@expo/vector-icons";
import * as Haptics from "expo-haptics";
import { Colors } from "@/constants/hive-stats";
  ...
  return (
    <View className="flex-row items-center justify-between py-0.5">
      <View className="flex-row items-center gap-2.5">
        <Text className="text-[15px] font-semibold text-[#191F28]">{data.condition}</Text>
        <View className="flex-row items-baseline">
          {data.high != null && data.low != null ? (
            <>
              <Text className="text-base font-bold text-[#191F28]">{data.high}°</Text>
              <Text className="text-[13px] text-[#B0B8C1] mx-0.5">/</Text>
              <Text className="text-[15px] font-medium text-[#B0B8C1]">{data.low}°</Text>
            </>
          ) : (
      ...
      <View className="flex-row items-center gap-1">
        <Text className="text-[13px] text-[#B0B8C1]">습도</Text>
...
function WeeklyWeather({ days }: { days: WeatherDay[] }) {
  ...
  const uniqueIcons = days
    .reduce<{ icon: string; iconColor: string }[]>((acc, w) => {
      if (!acc.find((a) => a.icon === w.icon)) acc.push({ icon: w.icon, iconColor: w.iconColor });
      return acc;
    }, [])
    .slice(0, 3);
    ...
    <>
      ...
      >
        <View className="flex-row items-center gap-2.5">
          ...
          <Text className="text-sm font-semibold text-[#191F28]">주간 날씨</Text>
          <Text className="text-[13px] text-[#8B95A1]">{dateRange}</Text>
        </View>
        <Feather name={expanded ? "chevron-up" : "chevron-down"} size={20} color={Colors.textTertiary} />
      </Pressable>
            ...
            {days.map((w, i) => (
              <View key={i}>
                <View className="flex-row items-center py-[7px] gap-3">
                  <Text className="text-sm font-semibold text-[#191F28] w-[26px] text-center">
                    {w.day}
                  </Text>
                  <Feather name={w.icon} size={20} color={w.iconColor} />
                  <Text className="text-sm text-[#B0B8C1] w-11 text-right">
                    {w.low != null ? `${w.low}°` : "-"}
                  </Text>
                  {/* Temp range bar */}
                  ...
                  <Text className="text-sm font-semibold text-[#191F28] w-11">
                    {w.high != null ? `${w.high}°` : "-"}
                  </Text>
                </View>
                {i < days.length - 1 && <View className="h-px bg-[#E5E8EB] opacity-50" />}
              </View>
            ))}
          </View>
        </>
    ...
    >
      <View className="flex-row items-center gap-2.5">
        <View className="flex-row items-center">
          ...
          <Feather name="cloud"      size={18} color="#90A4AE" style={{ marginLeft: -6 }} />
          <Feather name="cloud-rain" size={18} color="#64B5F6" style={{ marginLeft: -6 }} />
        </View>
        <Text className="text-[15px] font-semibold text-[#191F28]">월간 날씨</Text>
      </View>
      <View className="flex-row items-center gap-1">
        <Text className="text-sm text-[#3182F6]">보러 가기</Text>
...
function WeatherLoading() {
  return (
    <View className="flex-row items-center justify-center py-2 gap-2">
      ...
      <Text className="text-[13px] text-[#8B95A1]">날씨 불러오는 중...</Text>
    </View>
  );
}
...
function WeatherError({ msg }: { msg: string }) {
  return (
    <View className="flex-row items-center justify-center py-2 gap-2">
      ...
      <Text className="text-[13px] text-[#8B95A1]">{msg}</Text>
    </View>
  );
}

function WeatherEmpty({ text }: { text: string }) {
  return <Text className="text-[13px] text-[#8B95A1] text-center">{text}</Text>;
}
...
export function WeatherSection({
  period, stn, regionName, loading, errorMsg, todayWeather, weeklyWeather,
}: WeatherSectionProps) {
  return (
    <>
      ...
      <View className="flex-row items-center gap-1 mb-2">
        ...
        <Text className="text-[11px] text-[#B0B8C1]">{regionName} 기준</Text>
        <Text className="text-[11px] text-[#B0B8C1] flex-1 text-right">
          · 과거 날씨와 벌통 상태를 비교해 보세요!
        </Text>
      </View>

      {period === "일간" && (
        loading   ? <WeatherLoading /> :
        errorMsg  ? <WeatherError msg={errorMsg} /> :
        todayWeather
